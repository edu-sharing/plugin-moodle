...
class filter_edusharing extends moodle_text_filter {
	...
	public function filter($text, array $options = array())
	{
        ...
        //include script
        $PAGE->requires->js('/filter/edusharing/edu.js', true);        
...
// 			$text = '<html><head><meta http-equiv="Content-Type" content="text/html; charset=utf-8" /></head><body>'.$text.'</body></html>';
        ...
        //initialise jquery and helper function
        //do this here to serve all moodle versions (so far) and to encapsulate module
        //change this if all installations use moodle version >= 2.5
        $text = "<script>if(typeof jQuery=='undefined') {var headTag = document.getElementsByTagName('head')[0];var jqTag = document.createElement('script');jqTag.type = 'text/javascript';jqTag.src = '{$CFG->wwwroot}/mod/edusharing/js/jquery.min.js';headTag.appendChild(jqTag);}if(typeof loadAScript === 'undefined'){loadAScript = function(scriptname) {var snode = document.createElement('script');snode.type = 'text/javascript';snode.async = true;snode.src = scriptname.replace(/&amp;/g, '&');var s = document.getElementsByTagName('script')[0];s.parentNode.insertBefore(snode,s);}}</script>".$text;
	...
	protected function replaceRenderTokens(DOMNode $RenderNode, stdClass $edusharing)
	{
		...
		/*	if ( $Node->namespaceURI == 'http://edu-sharing.net/resource' )
			{
				switch( strtoupper($Node->localName) )
				{
					case 'TITLE':
						$Title = $Node->ownerDocument->createElement('span');
						$Node->parentNode->replaceChild($Title, $Node);

						$Title->nodeValue = $edusharing->name;
						$Title->setAttribute('class', 'title');

						break;

					default:
						error_log('Unhandled edu-sharing-node "'.$Node->localName.'"', E_WARNING);
				}
			}*/
		}
	}
	...
	protected function renderInline(stdClass $edusharing, $renderParams) {	    
		...
		$ticket = $ccauth->CCAuthenticationGetTicket($homePropArray['appid']);
		if ( ! $ticket ) {
		...
		$url = edusharing_get_redirect_url(
			$edusharing,
			$homePropArray,
			$repositoryPropArray,
			DISPLAY_MODE_INLINE);
            
        $url .= '&callback=get_resource_'.$edusharing->id;
		$inline = '<img src="'.$CFG->wwwroot.'/filter/edusharing/images/edu-loader.gif" />';
		try
		{
			session_write_close();

			error_log('Fetching inline-rendering from url "'.$url.'".');
            
            //add script to load resource via ajax
            $this->scripts[] = "function get_resource_".$edusharing->id."() {loadAScript('".$CFG->wwwroot."/filter/edusharing/proxy.php?URL=".urlencode($url)."&amp;resId=".$edusharing->id."&amp;title=".$renderParams['title']."&amp;mimetype=".$renderParams['mimetype']."')};function superGet_get_resource_".$edusharing->id."() {if(typeof jQuery=='undefined'){setTimeout(function(){superGet_get_resource_".$edusharing->id."()},100)}else{get_resource_".$edusharing->id."();};}superGet_get_resource_".$edusharing->id."();";
			session_start();
		}
		catch(Exception $e)
		{
...
// 			throw new Exception('Error loading resource from db.');
		...
		if ( $rendered )
		{
            ...
            switch($edusharing->window_float) {
                case 'left': $StyleAttr .= 'display: block; float: left; margin: 0 5px 5px 0;'; break;
                case 'right': $StyleAttr .= 'display: block; float: right; margin: 0 0 5px 5px;'; break;
                case 'inline': $StyleAttr .= 'display: inline-block; margin: 0 5px;'; break;
                case 'none':
                default: $StyleAttr .= 'display: block; float: none; margin: 5px 0;'; break;
            }
