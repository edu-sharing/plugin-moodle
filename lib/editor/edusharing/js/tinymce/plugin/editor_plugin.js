/**
 * edu-sharing Embed Plugin
 *
 * Copyright 2012, Hupfer Released under LGPL License.
 *
 */
	    ...
	    init : function(editor, url) {
		    ...
		    editor.addCommand('edusharing_insert_dialog', function() {
			    var dialog = {
				        ...
				        file:	editor.plugins.edusharing.dialog_url + 'insert.php',
		    ...
		    editor.addCommand('edusharing_edit_dialog', function(ui, value) {
			    ...
			    if (!editor.plugins.edusharing.handles(currentNode)) {
				    return false;
			    }
				    ...
				    if (1 == node.nodeType) {
				    	...
				    	if (node.getAttribute('es:resource_id')) {
						    var object = new edusharingObject(editor.getParam('edusharing_course_id'));
							    ...
							    //DELETE AFTER TESTING if ( node.getAttribute('src') )
							    // refresh ticket-param on preview-image
							    	
							    
                               if (node.getAttribute('src')) {
                            	   var previewUrl = node.getAttribute('src');
                            	   previewUrl = editor.plugins.edusharing.refreshTicketParam(previewUrl, editor.getParam('edusharing_ticket'));
                            	   if ( previewUrl ) 
                                      {
							    		node.setAttribute('src', previewUrl);
							          } else {
							    		alert('Error refreshing ticket-information.');
							    	 }//toggle with following for new preview service
                                /*
						    		previewUrl = editor.documentBaseURI.source + '/lib/editor/edusharing/preview.php';
						    		previewUrl += '?';
						    		previewUrl += 'objectUrl=' + encodeURIComponent(node.getAttribute('es:object_url'));
						    		previewUrl += '&';
							    ...
							    alert('Error importing node. Removing object.');
				    ...
				    if (1 == node.nodeType) {
					    ...
					    if (node.getAttribute('es:object_url')) {
						    ...
						    if (editor.plugins.edusharing.objects[resource_id]) {
							    ...
							    var object = new edusharingObject(editor.getParam('edusharing_course_id'));
							    if ( ! object.importNode(node)) {
								    alert('Error importing node.');
							    }
							    if (!object.link(node, editor, url)) {
								    alert('Error linking node. Removing inserted object.');
								    node.parentNode.removeChild(node);
							    }
