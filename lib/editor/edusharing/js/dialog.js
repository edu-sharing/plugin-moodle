...
var edusharingDialog = {
	...
	on_click_insert : function(form) {
		...
		if ( ! selectedNode ) {
			alert('No node selected so object can not be inserted here. Cancelling.');
			return false;
		}
		...
		if(mimeHelper == 'audio/' || mimeHelper == 'video/' || mimeHelper == 'image/' || form.repotype.value == 'YOUTUBE') {
			showPreview = true;
		}
	...
	preview_url : function(form) {
		...
		var preview_url = form.alfresco_base_url.value + '/../../edu-sharing/preview';
		preview_url = preview_url.concat('?nodeId=' + object_id);
		preview_url = preview_url.concat('&ticket=' + form.alfresco_ticket.value);
