...
var TEMPLATE = ' \
        ...
        <form id="edusharing_form" class="atto_form" style="display: none"> \
            ...
            <!--<div id="edusharing_wrapper_subtitle" class="edusharing_form_wrapper"> \
                ...
                <input type="checkbox" id="edusharing_version_display" name="edusharing_version_display" />\
                <label for="edusharing_version_display" class="edusharing_label_inline"> Immer letzte Version anzeigen</label> \
            <div id="edusharing_wrapper_alignment" class="edusharing_form_wrapper"> \
                <h2>Ausrichtung</h2> \
                <div class="edusharing_wrapper_alignment_radiowrapper"> \
                    <input type="radio" id="edusharing_alignment_left" name="edusharing_alignment" value="A"> \
                    <label for="edusharing_alignment_left" class="edusharing_label_inline">Links umfließend</label> \
                </div>\
                <div class="edusharing_wrapper_alignment_radiowrapper"> \
                    <input type="radio" id="edusharing_alignment_right" name="edusharing_alignment" value="B">\
                    <label for="edusharing_alignment_right" class="edusharing_label_inline">Rechts umfließend</label> \
                </div>\
                <div class="edusharing_wrapper_alignment_radiowrapper">\
                    <input type="radio" id="edusharing_alignment_none" name="edusharing_alignment" value="C" checked="checked">\
                    <label for="edusharing_alignment_none" class="edusharing_label_inline">Keine</label> \
                </div>\
            </div> \
            <div id="edusharing_wrapper_dimensions" class="edusharing_form_wrapper"> \
                <h2>Dimensionen</h2>\
                <div style="float:left;margin-right: 20px;">\
                    <label for="edusharing_width">Breite</label>\
                    <input type="number" id="edusharing_width" name="edusharing_width" value="" maxlength="4" length="4" /> px\
                </div> \
                <div>\
                    <label for="edusharing_height">Höhe</label> \
...
Y.namespace('M.atto_edusharing').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    updateDialog: function(node) {
        ...
        Y.one('#edusharing_title').setHTML(node.title || node.name);
        Y.one('#edusharing_title_display').setHTML(node.title || node.name);
        Y.one('#edusharing_width').set('value', node.properties['ccm:width']);
        Y.one('#edusharing_height').set('value', node.properties['ccm:height']);
        ...
        if(node.properties['ccm:width'] && node.properties['ccm:height'])
            Y.one('#edusharing_ratio').set('value', node.properties['ccm:width'] / node.properties['ccm:height']);
    ...
    _doInsert : function(e){
        ...
        var insert = '<img src="'+Y.one('#edusharing_preview_url').get('value')+'">';
