...
var TEMPLATE = '<div id="edusharing_hint"> ' +
    ...
    '<div class="edusharing_center edusharing_hint_buttons"><a id="edusharing_hint_close" class="btn" href="#">' +
...
Y.namespace('M.atto_edusharing').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
        ...
        while (0 < nodes.length) {
            ...
            if (1 == node.nodeType) {
                ...
                if (Y.one(node).hasClass('edusharing_atto')) {
                    ...
                    var qp = this.getUrlVars(query);

                    var object = new edusharingObject(this.get('courseid'));
                    if (object.importNode(qp)) {
                ...
                if (Y.one(node).hasClass('edusharing_atto')) {
                    ...
                    if (resource_id && this.get('existingObjects')[resource_id]) {
                        ...
                        if (!object.link(node)) {
                            ...
                            node.parentNode.removeChild(node); // /does not work     ?????check
                        }

                        if(node.nodeName.toLowerCase() == 'img') {
                            node.setAttribute('src', this.getPreviewUrl(object.id) + '&' + this.getQueryStringFromParams(qp));
                        } else {
                            node.setAttribute('href', this.getPreviewUrl(object.id) + '&' + this.getQueryStringFromParams(qp));
                        }
                    }
                }
            }
    ...
    handleUpdate: function() {
        ...
        if (selectedElement) {
            ...
            if (qp['mediatype'] == 'folder') {
                node.isDirectory = true;
            }
            if (qp['resourceId']) {
                node.resid = qp['resourceId'];
            }
            node.title = qp['title'];
            node.caption = qp['caption'];
            node.properties = [];
            if (qp['width']) {
            ...
            node.objectUrl = qp['object_url'];
            node.mimetype = qp['mimetype'];
            node.mediatype = qp['mediatype'];
            node.preview = [];
            ...
            node.properties['cclom:version'] = qp['window_version'];
    ...
    _doInsert: function(e) {
        ...
        if (node.resid) {
            ...
            if (this.getType(node.mediatype) == 'ref') {
                ...
                var url = selectedElement.attributes.href.value;
                url = this.replaceUrlParam(url, 'title', node.title);
                url = this.replaceUrlParam(url, 'caption', node.caption);
                selectedElement.setAttribute('href', url);
            } else {
                var url = selectedElement.attributes.src.value;
                url = this.replaceUrlParam(url, 'title', node.title);
                url = this.replaceUrlParam(url, 'caption', node.caption);
                url = this.replaceUrlParam(url, 'width', node.width);
                url = this.replaceUrlParam(url, 'height', node.height);
            ...
            if (node.type == 'ref') {
                insert = '&nbsp;<a ' + insert + ' href="'+url+'">' + node.title + '</a>&nbsp;';
            } else {
                insert += 'src="' + url +
                    '&width=' + node.width +
                    '&height=' + node.height + '"';
...
edusharingObject.prototype.importNode = function importNode(qp) {
    ...
    var object_version = qp['window_version'];
    if (object_version) {
