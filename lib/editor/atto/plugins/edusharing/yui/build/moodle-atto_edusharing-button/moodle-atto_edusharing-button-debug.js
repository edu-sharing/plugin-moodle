...
Y.namespace('M.atto_edusharing').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    getSelectedElement: function() {
        ...
        if (element && element.attributes && element.attributes['es:object_url']) {
            return element;
        }
    ...
    handleUpdate: function() {
        ...
        if (selectedElement) {
         ...
         node.isDirectory = true;
            ...
            if (selectedElement.attributes['es:resource_id']) {
        ...
        if (!update) {
            ...
            YUI().use('cookie', function(Y) {
                if (Y.Cookie.get("edusharing_hint_hide")) {
                    ...
                    that.open_repo();
                    YUI().use('node', function(Y) {Y.one('#edusharing_hint_check').setAttribute('checked', 'checked');});
                } else {
                    YUI().use('node', function(Y) {Y.one('#edusharing_hint_check').removeAttribute('checked');});
                }
            });
        }
