        ...
        handleUpdate: function() {
            ...
            if(selection[0].startContainer.childNodes.length > 0) {
                selection[0].startContainer.childNodes.forEach(function(element) {
                    if(element.classList.contains('edusharing'))
                        selectedElement = element;
                });
            }

            if(selection[0].startContainer.parentElement) {
                if(selection[0].startContainer.parentElement.classList.contains('edusharing'))
                    selectedElement = selection[0].startContainer.parentElement;
            }

//console.log(selection[0].startContainer.childNodes);
            ...
            if(selectedElement) {
                ...
                // on update hide fields that should not be edited
                // on submit get node in textarea and update only this fields
                // no usage update required! or?
                // some attributes are missing, e.g. always show latest, alignment
                // change submit button text / eventually dialog title
            }
        },
